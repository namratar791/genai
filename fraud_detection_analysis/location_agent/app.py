...
from models import A2ATaskRequest, A2ATaskResponse, AgentCard, AgentSkill, LocationOutput, FraudRequest

app = FastAPI(title="Location Agent")
graph = build_graph()


async def _run(req: FraudRequest) -> LocationOutput:
    state = {
    ...
    return LocationOutput(**result["analysis_result"])
...
async def agent_card() -> AgentCard:
    return AgentCard(
        name="location_agent",
        description="Analyzes geographic anomalies, impossible travel risk, and city mismatch.",
        endpoint="http://localhost:8102/tasks/send",
        version="1.0.0",
        skills=[
            AgentSkill(
                id="geo_analysis",
...
@app.post("/analyze", response_model=LocationOutput)
async def analyze(req: FraudRequest) -> LocationOutput:
